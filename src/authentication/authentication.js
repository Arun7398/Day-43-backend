...
router.post("/forgotPassword", async function (request, response) {
  try {
    ...
    const link = `https://password-reset-flow-react.netlify.app/passwordReset?token=${newToken}&id=${existingUser._id.toString()}`;
    ...
    return response
      .status(200)
      .send({ message: "Email password link sent your email" });
  } catch (error) {
